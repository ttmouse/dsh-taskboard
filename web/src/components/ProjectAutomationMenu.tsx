...
import type { AutomationHost } from "../api";
...
interface ProjectAutomationMenuProps {
  ...
  /** Reasonix 独立模式：隐藏 Codex 专属的额度/模型/推理强度选项 */
  reasonix?: boolean;
  onOpen: () => void;
...
export function ProjectAutomationMenu({
  ...
  useEffect(() => {
    if (!open) return;
    setDraft({ ...DEFAULT_OPTIONS, ...automation, host: automation?.host ?? "reasonix" });
  }, [open]);
  ...
  const menu = open ? createPortal(
    ...
    >
      ...
      {reasonix && (
        <label className="project-automation-field">
          <span>{text("执行宿主", "Execution host")}</span>
          <select
            value={draft.host}
            disabled={disabled}
            onChange={(event) => submitChange({
              ...draft,
              host: event.target.value as AutomationHost,
            })}
          >
            <option value="reasonix">{text("Reasonix", "Reasonix")}</option>
            <option value="dsh">{text("DeepSeek Harness", "DeepSeek Harness")}</option>
          </select>
        </label>
      )}
      {!reasonix && (
