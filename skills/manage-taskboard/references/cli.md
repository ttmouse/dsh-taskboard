...
Set `CODEX_TASKBOARD_URL` to override the default local API origin, `http://127.0.0.1:47824` (Reasonix fork default port).
...
Every issue or comment write must be attributed to a conversation. In Reasonix, `taskctl` reads the current conversation from `REASONIX_THREAD_ID` (falls back to `CODEX_THREAD_ID` for compatibility). Outside Reasonix, pass `--thread-id ID` explicitly. An explicit option takes precedence over the environment. Read commands do not require a conversation id.
...
Issues created through `taskctl` are assigned to the Reasonix Agent by default. Other CLI writes preserve the existing assignee.
...
Use either `--git-branch` or `--worktree-path`/`--worktree-branch`; an issue has only one development context. Issue JSON stores it as `developmentContext`, either `{ "type": "branch", "branch": "..." }` or `{ "type": "worktree", "path": "...", "branch": "..." }`. Its singular `threadId` is the conversation that most recently created or changed the issue itself. Recurrence requires a due date.
