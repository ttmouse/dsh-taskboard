...
function actorFromRequest(request) {
  if (request.headers["x-taskboard-client"] === "taskctl") {
    return agentActorFromRequest(request) ?? CODEX_AGENT_ACTOR;
  }
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (projectAutomationRoute) {
        ...
        if (!Number.isInteger(body.intervalMinutes ?? 10)
          || (body.intervalMinutes ?? 10) < 5
          || (body.intervalMinutes ?? 10) > 60) {
      ...
      if (taskRoute) {
        ...
        if (!action && request.method === "PATCH") {
          ...
          const task = database.updateTask(id, version, changes, threadId, actorFromRequest(request));
          events.emit("task.updated", { task });
          return sendJson(response, 200, { task });
        }
        if (action === "move" && request.method === "POST") {
          const move = parseMove(await readJson(request));
          const task = database.moveTask(id, move.version, move.status, move.sortOrder, move.threadId, actorFromRequest(request));
